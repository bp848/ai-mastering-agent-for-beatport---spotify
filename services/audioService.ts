...
export function resolveAdaptiveMasteringSettings(params: MasteringParams): {
  tubeHpfHz: number;
  exciterHpfHz: number;
  ...
  const safeNumber = (value: unknown, fallback: number): number => Number.isFinite(value) ? Number(value) : fallback;
  return {
    tubeHpfHz: Math.round(Math.max(20, Math.min(140, safeNumber(params.tube_hpf_hz, 30)))),
    exciterHpfHz: Math.round(Math.max(4000, Math.min(12000, safeNumber(params.exciter_hpf_hz, 6000)))),
    transientAttackS: Math.max(0.005, Math.min(0.05, safeNumber(params.transient_attack_s, 0.02))),
    transientReleaseS: Math.max(0.08, Math.min(0.6, safeNumber(params.transient_release_s, 0.25))),
    limiterAttackS: Math.max(0.0005, Math.min(0.005, safeNumber(params.limiter_attack_s, 0.002))),
    limiterReleaseS: Math.max(0.05, Math.min(0.3, safeNumber(params.limiter_release_s, 0.15))),
  };
}
